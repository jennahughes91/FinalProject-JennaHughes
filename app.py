...
# Formula weights (business area + product team + effort must sum to 1.0)
FORMULA_WEIGHTS = {
    "business_area": 0.45,
    "product_team":  0.35,
    "effort":        0.20,
...
# Flexible column name recognition — maps canonical name → list of accepted aliases
COLUMN_ALIASES = {
...
def normalize_priority(val, default: int = 3) -> int:
    ...
    if s.startswith("p") and s[1:].isdigit():
        p = int(s[1:])
        return max(1, min(5, 6 - p))  # P1→5, P5→1
    try:
        n = round(float(s))
...
def parse_backlog(file_bytes: bytes):
    ...
    for i, row in df_raw.iterrows():
        ...
        # Map every canonical field to its value (or None if column absent)
        ...
        # ── Effort ───────────────────────────────────────────────────────────
        item["effort_raw"] = item.get("effort")           # keep original for display
...
def compute_scores(df: pd.DataFrame, ba_weights: dict, pt_weights: dict) -> pd.DataFrame:
    ...
    def _score(row):
        ...
        return round(w["business_area"] * ba_s + w["product_team"] * pt_s + w["effort"] * eff_s, 4)
...
--- STRONG EXAMPLES (for calibration) ---
  "Add email-based password reset so users who forget their credentials can recover access without
   contacting support. Scope: email flow only, not SMS. Done when a user completes the full
   reset flow in under 2 minutes."
  "Migrate customer data exports to an async job queue to prevent API timeouts on exports over
...
def _evaluate_batch(client: anthropic.Anthropic, batch: list) -> list:
    ...
    items_json = json.dumps(
        [{"item_id": it["item_id"], "title": it["title"], "description": it["description"]} for it in batch],
        indent=2
    )
    ...
    # Extract the JSON array from the response (Claude may add preamble text)
    start = text.find("[")
    end   = text.rfind("]") + 1
    if start == -1 or end == 0:
...
def run_phase1_analysis(items_df: pd.DataFrame, progress_callback=None) -> dict:
    ...
    for idx, batch in enumerate(batches):
        ...
        try:
            ...
            # Graceful fallback: mark affected items as "review needed"
            for it in batch:
...
# ─────────────────────────────────────────────────────────────────────────────
# PRIORITY PROFILE MANAGEMENT  (persisted to local JSON file)
# ─────────────────────────────────────────────────────────────────────────────
...
def generate_export(ranked_df: pd.DataFrame, evaluations: dict) -> BytesIO:
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        # Row colouring based on description quality (column K = index 11)
...
with st.sidebar:
    ...
    backlog_file  = st.file_uploader("Backlog Excel *(required)*",  type=["xlsx", "xls"])
    capacity_file = st.file_uploader("Capacity Excel *(optional)*", type=["xlsx", "xls"])
    ...
    if backlog_file:
        ...
        if fh != st.session_state.file_hash:
            ...
            # Default weights: all areas/teams start at 3
...
if st.session_state.items_df is None:
    ...
    st.info(
        ...
        "| Effort | `Effort`, `Story Points`, `SP`, `Size`, `T-Shirt Size` (numeric or XS/S/M/L/XL) |\n"
    )
    st.stop()
